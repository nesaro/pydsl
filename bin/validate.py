...
def print_errors(postnode):
    ...
    if not postnode.valid:
        result += "Error at production: " + \
            str(postnode.production) + \
            "(" + str(postnode.leftpos) + \
            "," + str(postnode.rightpos) + ")"
...
def errors_to_list(postnode):
    ...
    if not postnode.valid:
        result.append(
            (postnode.leftpos, postnode.rightpos, str(postnode.production)))
        for child in postnode.childlist:
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "-i", "--inputfile", action="store", dest="inputfile", help="input filename ")
    PARSER.add_argument(
        "-e", "--expression", action="store", dest="expression", help="input expression")
    PARSER.add_argument('-o', dest='outputformat', nargs='?',
                        choices=["str", "json"], default="str", help="output formats")
