...
from ..Tree import ParserTreeNode
...
class RecursiveDescentResultTree:
    ...
    def __init__(self, content):
        if content != None and not isinstance(content, ParserTreeNode):
            raise TypeError
    ...
    def append(self, content, startpos):
        if not isinstance(content, ParserTreeNode):
            raise TypeError
    ...
    def __recursive_parser(self, onlysymbol, data, production, showerrors:bool = False, recurssions:int = 0):
        ...
        if isinstance(onlysymbol, TerminalSymbol):
            ...
            if showerrors and not result:
                ...
                return TypeCheckList(ParserTreeNode, [ParserTreeNode(0,len(data), [onlysymbol] , data, sproduction, valid = False)])
            return result
        elif isinstance(onlysymbol, NonTerminalSymbol):
            result = TypeCheckList(ParserTreeNode)
            tmpresults = []
            invalidlist = []
            ...
            for alternative in self._productionset.getProductionsBySide([onlysymbol]):
                for results in tmpresults:
                    ...
                    newresult = ParserTreeNode(0, results[-1].rightpos - results[0].leftpos, [onlysymbol], data[results[0].leftpos:results[-1].rightpos], production)
                    #print(newresult)
            ...
            if showerrors and not result:
                erroresult = TypeCheckList(ParserTreeNode, [ParserTreeNode(0,len(data), [onlysymbol] , data, production, valid = False)])
            ...
            return TypeCheckList(ParserTreeNode,[ParserTreeNode(0, 0, [onlysymbol], "", production)])
        else:
            raise Exception
