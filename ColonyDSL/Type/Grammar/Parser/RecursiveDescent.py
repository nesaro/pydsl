...
class RecursiveDescentResultTree:
    ...
    def __init__(self, content):
        if content != None and not isinstance(content, ParserTree):
            raise TypeError
    ...
    def append(self, content, startpos):
        if not isinstance(content, ParserTree):
            raise TypeError
    ...
    def __recursive_parser(self, onlysymbol, data, production, showerrors:bool = False, recurssions:int = 0):
        ...
        if isinstance(onlysymbol, TerminalSymbol):
            ...
            if showerrors and not result:
                ...
                return TypeCheckList(ParserTree, [ParserTree(0,len(data), [onlysymbol] , data, sproduction, valid = False)])
            return result
            ...
            for alternative in self._productionset.getProductionsBySide([onlysymbol]):
                for results in tmpresults:
                    ...
                    newresult = ParserTree(0, results[-1].rightpos - results[0].leftpos, [onlysymbol], data[results[0].leftpos:results[-1].rightpos], production)
                    #print(newresult)
            ...
            if showerrors and not result:
                erroresult = TypeCheckList(ParserTree, [ParserTree(0,len(data), [onlysymbol] , data, production, valid = False)])
            ...
            return TypeCheckList(ParserTree,[ParserTree(0, 0, [onlysymbol], "", production)])
        else:
            raise Exception
